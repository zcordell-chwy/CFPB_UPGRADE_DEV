...
RightNow.Widget.FilterDropdown2.prototype =
{
    /**
    ...
    */
    _onSelectChange: function(evt)
    {
        ...
        this._eo.filters.data.label = this._getSelectedLabel();

        if( !this._eo.filters.label )
            this._eo.filters.label = this.data.js.name;
    ...
    */
    ...
    /**
     * internal function to get the label of the selected option.
     */
    _getSelectedLabel: function()
    {
        if (this._optionsSelect)
        {
            ...
            // Return the selected label if and only if the index is valid.
            if( this._optionsSelect.options[index] && index > 0 )
            {
                return this._optionsSelect.options[index].text;
            }
        }
        return null;
    },

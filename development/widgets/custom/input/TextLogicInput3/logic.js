RightNow.Widget.TextLogicInput3 = function(data, instanceID) {
	this.data               = data;
	this.instanceID         = instanceID;
	this._formErrorLocation = null;
	this._validated         = false;
    this.validForNext       = false; // validation flag is set for nav widget to display complete/incomplete status
	...
	// Subscribe to toggleFormElement
	RightNow.Event.subscribe("evt_toggleFormElement", this._onFieldChange, this);
};

RightNow.Widget.TextLogicInput3.prototype = {
	/**
	 * ----------------------------------------------
	...
	_onValidate : function(type, args) {
		...
		this._validated = false;
        this.validForNext = this._validated;
		RightNow.Event.fire("evt_formFieldCountRequest");
	},
    ...
    _onFieldChange : function(type, args)
    {
    // only go through function if passed from same field (args[0].data.name == this._fieldName)
    var eoData = args[0].data;
    // only run if caller is from same field name
    if (eoData.name != this._fieldName)
        return;
	...
	hidingAncestor = (this.data.js.name == 'zip_sender' || this.data.js.name == 'zip_recipient') ? false : hidingAncestor;
	if(!hidingAncestor)
	{
		if(this.data.attrs.required_when_parent_equals && this.data.attrs.required_when_field_equals)
		{
            ...
            if(this.data.attrs.required_when_parent_equals == eoParent)
            {
                ...
                else
                {
                    ...
                    eo.data =
				    {
                        ...
                        "value" : this._inputField.value,
                        "table" : this.data.js.table,
                        "required" : this.data.attrs.required,
                        "prev" : this.data.js.prev,
                        "custom" : true
					};
                    RightNow.Event.fire("evt_toggleFormElement", eo);
                }
                this.hideShowReqLabel();
			}
		}
	}

        this.validForNext = this._blurValidate();
        // fire event for nav widget to listen
        var eoValid = new RightNow.Event.EventObject();
        eoValid.data = {
            "section" : this.data.attrs.section_id,
            "name" : this._fieldName,
            "valid" : this.validForNext
        };
        RightNow.Event.fire("evt_isValidForNext", eoValid);
    },
    
    hideShowReqLabel : function()
    {
	if(this.data.js.type == RightNow.Interface.Constants.EUF_DT_VARCHAR)
	{
		var spans = document.getElementById('rn_' + this.instanceID).getElementsByTagName('span');
		for(i = 0; i <= spans.length; i++)
		{
			if(YAHOO.util.Dom.hasClass(spans[i], 'rn_Required'))
			{
				if(this.data.attrs.required == false)
				{
					YAHOO.util.Dom.addClass(spans[i], 'rn_Hidden');
				}
				else
				{
					YAHOO.util.Dom.removeClass(spans[i], 'rn_Hidden');
				}
			}
		}
	}

...
class FileAttachmentUpload2DirectedRequests extends Widget
{
    ...
    function generateWidgetInformation()
    {
        ...
        $this->parms['comp_id'] = new UrlParam("ComplaintAgainstOrg ID", 'comp_id', false, "Get complaint against org id", 'comp_id/7');
    }

    function getData()
    {
        $compId = $this->parms['comp_id'];
        if ($compId) {
            $this->CI->load->model('custom/instagent_model');
            $this->parms['i_id'] = $this->CI->instagent_model->getComplaint($compId)->Incident->ID;
            
            $this->CI->load->model('custom/dr_model');
            $investigation = $this->CI->dr_model->getInvestigationByIncidentComplaint($this->parms['i_id'], $compId);
            $this->parms['investigation_id'] = $investigation->ID;
            //Check if message already has max number of file attachments

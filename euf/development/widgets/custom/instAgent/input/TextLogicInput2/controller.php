...
class TextLogicInput2 extends FormInput
{
    function __construct()
    {
        ...
        $this -> attrs['regex_mask'] = new Attribute( 'Regular Expression Mask', 'STRING', "Regular expression used to validate form field value entered by user. Defaults to empty string, which indicates pattern matching will not be attempted. Pattern matching applied in widget's javascript.", '' );
		 $this->attrs['label_input_sub'] = new Attribute("Label subtext", 'STRING', "Provides additional detail under label", false);
		$this -> attrs['max_age'] = new Attribute( 'Maximum Age', 'INT', "Maximum age a date can be. Defaults to zero, which will do no validation of known date fields (incidents.c$date_birth).", 0 );
    }

...
class NavigationTab2 extends Widget
{
    function __construct()
    {
        ...
        $this->attrs['alt_link'] = new Attribute('Alternate Link', 'STRING', "Alternate link if user isn't allowed to access the link passed in. User access is only checked if both the alt_link and allowed_user_types attribures are not null.", null);
        $this->attrs['allowed_user_types'] = new Attribute('Allowed User Types', 'STRING', "Comma-delimited list of user types (i.e. company, federal, state) allowed to access the link. User access is only checked if both the alt_link and allowed_user_types attribures are not null.", null);
    ...
    function getData()
    {
        ...
        if( isset( $this->data['attrs']['allowed_user_types'] ) && isset( $this->data['attrs']['alt_link'] ) )
        {
            $this->CI->load->model( 'custom/ContactPermissions_model' );
            $currentUserType = $this->CI->ContactPermissions_model->userType();
            $legalUsers = explode( ',', $this->data['attrs']['allowed_user_types'] );
            ...
            if( $userAllowedAccess )
                $this->data['link'] = $this->data['attrs']['link'];
            else
                $this->data['link'] = $this->data['attrs']['alt_link'];
        }
        else
        {
            $this->data['link'] = $this->data['attrs']['link'];
